...
<html>
    ...
    <body style="background-image: url('images/background.png'); background-size: cover;" >
        ...
        <script>
            var numOfCoregulatorsShow = new Map;
            var count = -1;
            var organisms = [];
        </script>
            ...
            <c:forEach items="${numOfCoregulators}" var="numOfGeneCoregulators">
                ...
                <div class="container-fluid" style="margin-bottom: 80px;">
                    <div class="row font">
                        <c:choose>
                            <c:when test="${count==0}">
                                <div class="col-sm-12"><p class="dist-info">Distribution of the number of co-regulating transcription factors (${type} database)</p></div>
                            </c:when>
                            <c:otherwise>
                                <div class="col-sm-12"><center><span class="dist-info">Distribution of the number of co-regulating transcription factors for <a href="organismInfo.htm?id=${organismsId.get(numOfGeneCoregulators.key)}&type=${type}">${numOfGeneCoregulators.key}</a></span></center></div>
                                        </c:otherwise>
                                    </c:choose>
                    </div>
                ...
                <script>
                    var key = '${numOfGeneCoregulators.key}';
                    var value = '${numOfGeneCoregulators.value}';
                    ...
                    numOfCoregulatorsShow.set(key, coregulatorsObject);
                    var auxRetriver = {};
                    auxRetriver = numOfCoregulatorsShow.get(key);
                    ...
                    function coregulatorsObjectFunction(value) {
                        ...
                        for (var i = 0; i < aux.length; i++) {
                            ...
                            if (attributes[0].includes("{")) {
                                ...
                                //coregulators.push(attributes[0]);
                            }
...
//                                if (attributesAux[0].length == 1) {                                    
                                //tfs.push("00" + attributesAux[0]);
                                //                              } else if (attributesAux[0].length == 2) {
                                //tfs.push("0" + attributesAux[0]);
                                //                            } else {
                                //                              tfs.push(attributesAux[0]);
                                //                        }
                            } else {
                                numOfTfs = attributes[1];
                                //  if (attributes[1].length == 1) {
                                //  tfs.push("00" + attributes[1]);
                                //} else if (attributes[1].length == 2) {
                                //  tfs.push("0" + attributes[1]);
                                //} else {
                                //    tfs.push(attributes[1]);
                                //  }
                                //}
                            }
            ...
            <script>
                for (var i = 0; i < numOfCoregulatorsShow.size; i++) {
                    count++;
                    ...
                    for (var k = 0; k < numOfCoregulatorsShow.get(organisms[i]).tfs.length; k++) {
                        //console.log("numOfCoregulatorsShow.get(organisms[i]).coregulators[k]: " + numOfCoregulatorsShow.get(organisms[i]).coregulators[k]);
                        //console.log("numOfCoregulatorsShow.get(organisms[i]).tfs[k]: " + numOfCoregulatorsShow.get(organisms[i]).tfs[k]);
                        var numOfCoregulatorsAux = [];
                        numOfCoregulatorsAux.coregulators = numOfCoregulatorsShow.get(organisms[i]).coregulators[k];
                        numOfCoregulatorsAux.tfs = numOfCoregulatorsShow.get(organisms[i]).tfs[k];
